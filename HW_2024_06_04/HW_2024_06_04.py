...
# 1. Использование словаря для хранения книг: 
# в классе `Department` можно хранить книги в словаре с ключом, равным имени книги.
# Это упростит поиск и удаление книг, а также предотвратит дублирование.


# 2. Я про это не рассказывал, но можно изучить самостоятельно при наличии времени.
# Вместо перебора всех отделов и книг в методе find_book класса Library,
# можно использовать метод itertools.chain для объединения всех книг из разных отделов в один итерируемый объект,
# а затем проверить, существует ли книга с заданным именем. Это упростит код и улучшит производительность.
# Вот пример реализации метода find_book с использованием itertools.chain:
...
#         Возвращает книгу, в противном случае поднимает ошибку, что книга не найдена ни в одном департаменте
#         """
#         books = itertools.chain(*[department.books for department in self.departments.values()])
#         for book in books:
#             if book.name == book_name:
#                 return book
#         raise BookNotFoundError(book_name, "any department")
...
# 3. Добавление методов для проверки наличия книг: 
# в классе Library можно вынести отдельно методы для проверки,
# существует ли книга в библиотеке или в конкретном отделе.


# 4. Добавление методов для сортировки книг:
# в классе Department можно добавить методы для сортировки книг по разным критериям,
# например, по алфавиту, году издания или автору.


# 5. Я про это не рассказывал, но можно изучить самостоятельно при наличии времени.
# Использование контекстного менеджера для работы с библиотекой:
# можно реализовать контекстный менеджер для класса Library,
# который будет автоматически выдавать и возвращать книги,
# а также выводить информацию о библиотеке и читателях.

# Контекстный менеджер (context manager) в Python — это объект, который управляет ресурсами,
# выделенными для блока кода, и освобождает их после выхода из блока.
# Контекстный менеджер определяется с помощью методов __enter__ и __exit__.

# В классе Library можно реализовать контекстный менеджер для автоматической выдачи и возврата книг,
# а также вывода информации о библиотеке и читателях.
# Это упростит работу с библиотекой и сделает код более читабельным.

# Вот пример реализации контекстного менеджера для класса Library:
...
# Вы отлично справились с заданием. Не воспринимайте мой комментарий как критику.
# Это просто пример того что можно сделать ещё если бы у нас было время и возможность =).
